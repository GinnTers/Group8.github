function loadAgeSpendingChart() {
    ...
    d3.json("/visualize/").then(function(data) {
      ...
      svg.append("text")
        ...
        .style("fill", "#00A896")
        .text("Doanh thu trung bình theo Độ tuổi");
  
      // Xử lý dữ liệu
      // Nhóm theo độ tuổi, tính tổng chi tiêu và số khách hàng duy nhất
      const spendingByAge = d3.rollup(data,
        v => ({
      ...
      const dataset = Array.from(spendingByAge, ([ageGroup, values]) => {
        ...
        return { ageGroup, avgSpending, category };
      ...
      d3.select("#insights").html(`
        ...
        <p>Nhóm độ tuổi "${highest.ageGroup}" có doanh thu trung bình cao nhất, đạt ${Math.round(highest.avgSpending / 1000)}K VND, thuộc phân loại "${highest.category}".</p>
        <p>Nhóm độ tuổi "${lowest.ageGroup}" có doanh thu trung bình thấp nhất, chỉ ${Math.round(lowest.avgSpending / 1000)}K VND, thuộc phân loại "${lowest.category}".</p>
        <p>Xu hướng này cho thấy sự khác biệt về khả năng chi tiêu giữa các nhóm độ tuổi, có thể do thu nhập và thói quen mua sắm khác nhau.</p>
      `);
    });
  }